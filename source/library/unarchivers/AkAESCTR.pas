...
type
  TCTRExpandedKey = array[0..44, 0..4] of byte;
  TCTRMessage = array[0..15] of byte;
  TCTRState = array[0..3, 0..3] of byte;
  TCTRQWord = array[0..3] of byte;

    {
        Implements the simplest form of AES-128 encryption in CTR (counter)
        mode. It is used for secure key generation from a string key when we
        are encrypting/decrypting using AES-128 in CBC mode.
    }
  TAESCTR = class
  private
    function AddRoundKey(state: TCTRState; w: TCTRExpandedKey;
      rnd: byte; Nb: byte): TCTRState;
    function SubBytes(s: TCTRState; Nb: byte): TCTRState;
    function ShiftRows(s: TCTRState; Nb: byte): TCTRState;
    function MixColumns(s: TCTRState; Nb: byte): TCTRState;
    function SubWord(w: TCTRQWord): TCTRQWord;
    function RotWord(w: TCTRQWord): TCTRQWord;
  public
    function Cipher(input: TCTRMessage; w: TCTRExpandedKey): TCTRMessage;
    function KeyExpansion(key: TCTRMessage): TCTRExpandedKey;
    function makeKey(password: string): TCTRMessage;
  end;
  ...
  private
    ...
    iv: TAESBuffer;
  public
    procedure AESSetPassword(password: string);
    function AESDecryptCBC(sFrom: TStream): TMemoryStream;
  end;
...
function TAESCTR.AddRoundKey(state: TCTRState; w: TCTRExpandedKey;
  rnd, Nb: byte): TCTRState;
var
  r, c: integer;
begin
  for r := 0 to 3 do
    for c := 0 to Nb - 1 do
      state[r][c] := state[r][c] xor w[rnd * 4 + c][r];
  Result := state;
end;

function TAESCTR.Cipher(input: TCTRMessage; w: TCTRExpandedKey): TCTRMessage;
var
  Nb, Nr: byte;
  state: TCTRState;
  round, i: integer;
begin
  ...
  for i := 0 to 4 * Nb - 1 do
    state[i mod 4][Floor(i / 4)] := input[i];
  ...
  for i := 0 to 4 * Nb - 1 do
    Result[i] := state[i mod 4][floor(i / 4)];
end;

function TAESCTR.KeyExpansion(key: TCTRMessage): TCTRExpandedKey;
var
  i, t: integer;
  Nb, Nr, Nk: byte;
  w: TCTRExpandedKey;
...
begin
  ...
  begin
    ...
    begin
      ...
      for t := 0 to 3 do
        temp[t] := temp[t] xor RCon[Round(i / Nk)][t];
    end
    else if ((Nk > 6) and (i mod Nk = 4)) then
      temp := SubWord(temp);

    for t := 0 to 3 do
      w[i][t] := w[i - Nk][t] xor temp[t];
  end;

  Result := w;
end;

function TAESCTR.makeKey(password: string): TCTRMessage;
var
  pwBytes: TCTRMessage;
  i: integer;
begin
  if (Length(password) > 0) then
    for i := 0 to Min(15, Length(password)) do
      pwBytes[i] := Ord(password[i + 1]) and $ff;
  if (Length(password) < 15) then
    for i := Length(password) to 15 do
      pwBytes[i] := 0;

  Result := Cipher(pwBytes, KeyExpansion(pwBytes));
end;

function TAESCTR.MixColumns(s: TCTRState; Nb: byte): TCTRState;
var
...
begin
  ...
  begin
    ...
    begin
      a[i] := s[i][c];
      if ((s[i][c] and $80) > 0) then
        b[i] := (s[i][c] shl 1) xor $011b
      else
        b[i] := s[i][c] shl 1;
    end;
    s[0][c] := b[0] xor a[1] xor b[1] xor a[2] xor a[3];
    s[1][c] := a[0] xor b[1] xor a[2] xor b[2] xor a[3];
    s[2][c] := a[0] xor a[1] xor b[2] xor a[3] xor b[3];
    s[3][c] := a[0] xor b[0] xor a[1] xor a[2] xor b[3];
  end;
  Result := s;
end;

function TAESCTR.RotWord(w: TCTRQWord): TCTRQWord;
var
  tmp: byte;
  i: integer;
begin
...
function TAESCTR.ShiftRows(s: TCTRState; Nb: byte): TCTRState;
var
  t: array[0..3] of byte;
  r, c: integer;
begin
...
function TAESCTR.SubBytes(s: TCTRState; Nb: byte): TCTRState;
var
  r, c: integer;
begin
  for r := 0 to 3 do
...
function TAESCTR.SubWord(w: TCTRQWord): TCTRQWord;
var
  i: integer;
begin
  for i := 0 to 3 do
...
const
  ...
  InverseTable: array [0..255] of longword = (
    ...
    $AB58FAAC, $9303E34B,
    $55FA3020, $F66D76AD, $9176CC88, $254C02F5, $FCD7E54F, $D7CB2AC5,
    $80443526, $8FA362B5,
    $495AB1DE, $671BBA25, $980EEA45, $E1C0FE5D, $02752FC3, $12F04C81,
    $A397468D, $C6F9D36B,
    $E75F8F03, $959C9215, $EB7A6DBF, $DA595295, $2D83BED4, $D3217458,
    $2969E049, $44C8C98E,
    $6A89C275, $78798EF4, $6B3E5899, $DD71B927, $B64FE1BE, $17AD88F0,
    $66AC20C9, $B43ACE7D,
    $184ADF63, $82311AE5, $60335197, $457F5362, $E07764B1, $84AE6BBB,
    $1CA081FE, $942B08F9,
    $58684870, $19FD458F, $876CDE94, $B7F87B52, $23D373AB, $E2024B72,
    $578F1FE3, $2AAB5566,
    $0728EBB2, $03C2B52F, $9A7BC586, $A50837D3, $F2872830, $B2A5BF23,
    $BA6A0302, $5C8216ED,
    $2B1CCF8A, $92B479A7, $F0F207F3, $A1E2694E, $CDF4DA65, $D5BE0506,
    $1F6234D1, $8AFEA6C4,
    $9D532E34, $A055F3A2, $32E18A05, $75EBF6A4, $39EC830B, $AAEF6040,
    $069F715E, $51106EBD,
    $F98A213E, $3D06DD96, $AE053EDD, $46BDE64D, $B58D5491, $055DC471,
    $6FD40604, $FF155060,
    $24FB9819, $97E9BDD6, $CC434089, $779ED967, $BD42E8B0, $888B8907,
    $385B19E7, $DBEEC879,
    $470A7CA1, $E90F427C, $C91E84F8, $00000000, $83868009, $48ED2B32,
    $AC70111E, $4E725A6C,
    $FBFF0EFD, $5638850F, $1ED5AE3D, $27392D36, $64D90F0A, $21A65C68,
    $D1545B9B, $3A2E3624,
    $B1670A0C, $0FE75793, $D296EEB4, $9E919B1B, $4FC5C080, $A220DC61,
    $694B775A, $161A121C,
    $0ABA93E2, $E52AA0C0, $43E0223C, $1D171B12, $0B0D090E, $ADC78BF2,
    $B9A8B62D, $C8A91E14,
    $8519F157, $4C0775AF, $BBDD99EE, $FD607FA3, $9F2601F7, $BCF5725C,
    $C53B6644, $347EFB5B,
    $7629438B, $DCC623CB, $68FCEDB6, $63F1E4B8, $CADC31D7, $10856342,
    $40229713, $2011C684,
    $7D244A85, $F83DBBD2, $1132F9AE, $6DA129C7, $4B2F9E1D, $F330B2DC,
    $EC52860D, $D0E3C177,
    $6C16B32B, $99B970A9, $FA489411, $2264E947, $C48CFCA8, $1A3FF0A0,
    $D82C7D56, $EF903322,
    $C74E4987, $C1D138D9, $FEA2CA8C, $360BD498, $CF81F5A6, $28DE7AA5,
    $268EB7DA, $A4BFAD3F,
    $E49D3A2C, $0D927850, $9BCC5F6A, $62467E54, $C2138DF6, $E8B8D890,
    $5EF7392E, $F5AFC382,
    $BE805D9F, $7C93D069, $A92DD56F, $B31225CF, $3B99ACC8, $A77D1810,
    $6E639CE8, $7BBB3BDB,
    $097826CD, $F418596E, $01B79AEC, $A89A4F83, $656E95E6, $7EE6FFAA,
    $08CFBC21, $E6E815EF,
    $D99BE7BA, $CE366F4A, $D4099FEA, $D67CB029, $AFB2A431, $31233F2A,
    $3094A5C6, $C066A235,
    $37BC4E74, $A6CA82FC, $B0D090E0, $15D8A733, $4A9804F1, $F7DAEC41,
    $0E50CD7F, $2FF69117,
    $8DD64D76, $4DB0EF43, $544DAACC, $DF0496E4, $E3B5D19E, $1B886A4C,
    $B81F2CC1, $7F516546,
    $04EA5E9D, $5D358C01, $737487FA, $2E410BFB, $5A1D67B3, $52D2DB92,
    $335610E9, $1347D66D,
    $8C61D79A, $7A0CA137, $8E14F859, $893C13EB, $EE27A9CE, $35C961B7,
    $EDE51CE1, $3CB1477A,
    $59DFD29C, $3F73F255, $79CE1418, $BF37C773, $EACDF753, $5BAAFD5F,
    $146F3DDF, $86DB4478,
    $81F3AFCA, $3EC468B9, $2C342438, $5F40A3C2, $72C31D16, $0C25E2BC,
    $8B493C28, $41950DFF,
    $7101A839, $DEB30C08, $9CE4B4D8, $90C15664, $6184CB7B, $70B632D5,
    $745C6C48, $4257B8D0
    );
  ...
  LastInverseTable: array [0..255] of longword = (
    ...
    $000000A5, $00000038,
    $000000BF, $00000040, $000000A3, $0000009E, $00000081, $000000F3,
    $000000D7, $000000FB,
    $0000007C, $000000E3, $00000039, $00000082, $0000009B, $0000002F,
    $000000FF, $00000087,
    $00000034, $0000008E, $00000043, $00000044, $000000C4, $000000DE,
    $000000E9, $000000CB,
    $00000054, $0000007B, $00000094, $00000032, $000000A6, $000000C2,
    $00000023, $0000003D,
    $000000EE, $0000004C, $00000095, $0000000B, $00000042, $000000FA,
    $000000C3, $0000004E,
    $00000008, $0000002E, $000000A1, $00000066, $00000028, $000000D9,
    $00000024, $000000B2,
    $00000076, $0000005B, $000000A2, $00000049, $0000006D, $0000008B,
    $000000D1, $00000025,
    $00000072, $000000F8, $000000F6, $00000064, $00000086, $00000068,
    $00000098, $00000016,
    $000000D4, $000000A4, $0000005C, $000000CC, $0000005D, $00000065,
    $000000B6, $00000092,
    $0000006C, $00000070, $00000048, $00000050, $000000FD, $000000ED,
    $000000B9, $000000DA,
    $0000005E, $00000015, $00000046, $00000057, $000000A7, $0000008D,
    $0000009D, $00000084,
    $00000090, $000000D8, $000000AB, $00000000, $0000008C, $000000BC,
    $000000D3, $0000000A,
    $000000F7, $000000E4, $00000058, $00000005, $000000B8, $000000B3,
    $00000045, $00000006,
    $000000D0, $0000002C, $0000001E, $0000008F, $000000CA, $0000003F,
    $0000000F, $00000002,
    $000000C1, $000000AF, $000000BD, $00000003, $00000001, $00000013,
    $0000008A, $0000006B,
    $0000003A, $00000091, $00000011, $00000041, $0000004F, $00000067,
    $000000DC, $000000EA,
    $00000097, $000000F2, $000000CF, $000000CE, $000000F0, $000000B4,
    $000000E6, $00000073,
    $00000096, $000000AC, $00000074, $00000022, $000000E7, $000000AD,
    $00000035, $00000085,
    $000000E2, $000000F9, $00000037, $000000E8, $0000001C, $00000075,
    $000000DF, $0000006E,
    $00000047, $000000F1, $0000001A, $00000071, $0000001D, $00000029,
    $000000C5, $00000089,
    $0000006F, $000000B7, $00000062, $0000000E, $000000AA, $00000018,
    $000000BE, $0000001B,
    $000000FC, $00000056, $0000003E, $0000004B, $000000C6, $000000D2,
    $00000079, $00000020,
    $0000009A, $000000DB, $000000C0, $000000FE, $00000078, $000000CD,
    $0000005A, $000000F4,
    $0000001F, $000000DD, $000000A8, $00000033, $00000088, $00000007,
    $000000C7, $00000031,
    $000000B1, $00000012, $00000010, $00000059, $00000027, $00000080,
    $000000EC, $0000005F,
    $00000060, $00000051, $0000007F, $000000A9, $00000019, $000000B5,
    $0000004A, $0000000D,
    $0000002D, $000000E5, $0000007A, $0000009F, $00000093, $000000C9,
    $0000009C, $000000EF,
    $000000A0, $000000E0, $0000003B, $0000004D, $000000AE, $0000002A,
    $000000F5, $000000B0,
    $000000C8, $000000EB, $000000BB, $0000003C, $00000083, $00000053,
    $00000099, $00000061,
    $00000017, $0000002B, $00000004, $0000007E, $000000BA, $00000077,
    $000000D6, $00000026,
    $000000E1, $00000069, $00000014, $00000063, $00000055, $00000021,
    $0000000C, $0000007D
    );
...
begin
  ...
  repeat
    ...
    ExpandedKey[I + 4] := ExpandedKey[I] xor
      ...
      ((W3 shl 24) or (W3 shr 8))) xor Rcon2[J];
    Inc(J);
...
begin
  ...
  W3 := ForwardTable[byte(T0[3] shr 24)];
  T1[0] := (W0 xor ((W1 shl 8) or (W1 shr 24)) xor ((W2 shl 16) or (W2 shr 16)) xor
    ((W3 shl 24) or (W3 shr 8))) xor Key[12];
  W0 := ForwardTable[byte(T0[1])];
  W1 := ForwardTable[byte(T0[2] shr 8)];
  W2 := ForwardTable[byte(T0[3] shr 16)];
  W3 := ForwardTable[byte(T0[0] shr 24)];
  T1[1] := (W0 xor ((W1 shl 8) or (W1 shr 24)) xor ((W2 shl 16) or (W2 shr 16)) xor
    ((W3 shl 24) or (W3 shr 8))) xor Key[13];
  W0 := ForwardTable[byte(T0[2])];
  W1 := ForwardTable[byte(T0[3] shr 8)];
  W2 := ForwardTable[byte(T0[0] shr 16)];
  W3 := ForwardTable[byte(T0[1] shr 24)];
  T1[2] := (W0 xor ((W1 shl 8) or (W1 shr 24)) xor ((W2 shl 16) or (W2 shr 16)) xor
    ((W3 shl 24) or (W3 shr 8))) xor Key[14];
  W0 := ForwardTable[byte(T0[3])];
  W1 := ForwardTable[byte(T0[0] shr 8)];
  W2 := ForwardTable[byte(T0[1] shr 16)];
  W3 := ForwardTable[byte(T0[2] shr 24)];
  T1[3] := (W0 xor ((W1 shl 8) or (W1 shr 24)) xor ((W2 shl 16) or (W2 shr 16)) xor
    ((W3 shl 24) or (W3 shr 8))) xor Key[15];
  // round 4
  W0 := ForwardTable[byte(T1[0])];
  W1 := ForwardTable[byte(T1[1] shr 8)];
  W2 := ForwardTable[byte(T1[2] shr 16)];
  W3 := ForwardTable[byte(T1[3] shr 24)];
  T0[0] := (W0 xor ((W1 shl 8) or (W1 shr 24)) xor ((W2 shl 16) or (W2 shr 16)) xor
    ((W3 shl 24) or (W3 shr 8))) xor Key[16];
  W0 := ForwardTable[byte(T1[1])];
  W1 := ForwardTable[byte(T1[2] shr 8)];
  W2 := ForwardTable[byte(T1[3] shr 16)];
  W3 := ForwardTable[byte(T1[0] shr 24)];
  T0[1] := (W0 xor ((W1 shl 8) or (W1 shr 24)) xor ((W2 shl 16) or (W2 shr 16)) xor
    ((W3 shl 24) or (W3 shr 8))) xor Key[17];
  W0 := ForwardTable[byte(T1[2])];
  W1 := ForwardTable[byte(T1[3] shr 8)];
  W2 := ForwardTable[byte(T1[0] shr 16)];
  W3 := ForwardTable[byte(T1[1] shr 24)];
  T0[2] := (W0 xor ((W1 shl 8) or (W1 shr 24)) xor ((W2 shl 16) or (W2 shr 16)) xor
    ((W3 shl 24) or (W3 shr 8))) xor Key[18];
  W0 := ForwardTable[byte(T1[3])];
  W1 := ForwardTable[byte(T1[0] shr 8)];
  W2 := ForwardTable[byte(T1[1] shr 16)];
  W3 := ForwardTable[byte(T1[2] shr 24)];
  T0[3] := (W0 xor ((W1 shl 8) or (W1 shr 24)) xor ((W2 shl 16) or (W2 shr 16)) xor
    ((W3 shl 24) or (W3 shr 8))) xor Key[19];
  // round 5
  W0 := ForwardTable[byte(T0[0])];
  W1 := ForwardTable[byte(T0[1] shr 8)];
  W2 := ForwardTable[byte(T0[2] shr 16)];
  W3 := ForwardTable[byte(T0[3] shr 24)];
  T1[0] := (W0 xor ((W1 shl 8) or (W1 shr 24)) xor ((W2 shl 16) or (W2 shr 16)) xor
    ((W3 shl 24) or (W3 shr 8))) xor Key[20];
  W0 := ForwardTable[byte(T0[1])];
  W1 := ForwardTable[byte(T0[2] shr 8)];
  W2 := ForwardTable[byte(T0[3] shr 16)];
  W3 := ForwardTable[byte(T0[0] shr 24)];
  T1[1] := (W0 xor ((W1 shl 8) or (W1 shr 24)) xor ((W2 shl 16) or (W2 shr 16)) xor
    ((W3 shl 24) or (W3 shr 8))) xor Key[21];
  W0 := ForwardTable[byte(T0[2])];
  W1 := ForwardTable[byte(T0[3] shr 8)];
  W2 := ForwardTable[byte(T0[0] shr 16)];
  W3 := ForwardTable[byte(T0[1] shr 24)];
  T1[2] := (W0 xor ((W1 shl 8) or (W1 shr 24)) xor ((W2 shl 16) or (W2 shr 16)) xor
    ((W3 shl 24) or (W3 shr 8))) xor Key[22];
  W0 := ForwardTable[byte(T0[3])];
  W1 := ForwardTable[byte(T0[0] shr 8)];
  W2 := ForwardTable[byte(T0[1] shr 16)];
  W3 := ForwardTable[byte(T0[2] shr 24)];
  T1[3] := (W0 xor ((W1 shl 8) or (W1 shr 24)) xor ((W2 shl 16) or (W2 shr 16)) xor
    ((W3 shl 24) or (W3 shr 8))) xor Key[23];
  // round 6
  W0 := ForwardTable[byte(T1[0])];
  W1 := ForwardTable[byte(T1[1] shr 8)];
  W2 := ForwardTable[byte(T1[2] shr 16)];
  W3 := ForwardTable[byte(T1[3] shr 24)];
  T0[0] := (W0 xor ((W1 shl 8) or (W1 shr 24)) xor ((W2 shl 16) or (W2 shr 16)) xor
    ((W3 shl 24) or (W3 shr 8))) xor Key[24];
  W0 := ForwardTable[byte(T1[1])];
  W1 := ForwardTable[byte(T1[2] shr 8)];
  W2 := ForwardTable[byte(T1[3] shr 16)];
  W3 := ForwardTable[byte(T1[0] shr 24)];
  T0[1] := (W0 xor ((W1 shl 8) or (W1 shr 24)) xor ((W2 shl 16) or (W2 shr 16)) xor
    ((W3 shl 24) or (W3 shr 8))) xor Key[25];
  W0 := ForwardTable[byte(T1[2])];
  W1 := ForwardTable[byte(T1[3] shr 8)];
  W2 := ForwardTable[byte(T1[0] shr 16)];
  W3 := ForwardTable[byte(T1[1] shr 24)];
  T0[2] := (W0 xor ((W1 shl 8) or (W1 shr 24)) xor ((W2 shl 16) or (W2 shr 16)) xor
    ((W3 shl 24) or (W3 shr 8))) xor Key[26];
  W0 := ForwardTable[byte(T1[3])];
  W1 := ForwardTable[byte(T1[0] shr 8)];
  W2 := ForwardTable[byte(T1[1] shr 16)];
  W3 := ForwardTable[byte(T1[2] shr 24)];
  T0[3] := (W0 xor ((W1 shl 8) or (W1 shr 24)) xor ((W2 shl 16) or (W2 shr 16)) xor
    ((W3 shl 24) or (W3 shr 8))) xor Key[27];
  // round 7
  W0 := ForwardTable[byte(T0[0])];
  W1 := ForwardTable[byte(T0[1] shr 8)];
  W2 := ForwardTable[byte(T0[2] shr 16)];
  W3 := ForwardTable[byte(T0[3] shr 24)];
  T1[0] := (W0 xor ((W1 shl 8) or (W1 shr 24)) xor ((W2 shl 16) or (W2 shr 16)) xor
    ((W3 shl 24) or (W3 shr 8))) xor Key[28];
  W0 := ForwardTable[byte(T0[1])];
  W1 := ForwardTable[byte(T0[2] shr 8)];
  W2 := ForwardTable[byte(T0[3] shr 16)];
  W3 := ForwardTable[byte(T0[0] shr 24)];
  T1[1] := (W0 xor ((W1 shl 8) or (W1 shr 24)) xor ((W2 shl 16) or (W2 shr 16)) xor
    ((W3 shl 24) or (W3 shr 8))) xor Key[29];
  W0 := ForwardTable[byte(T0[2])];
  W1 := ForwardTable[byte(T0[3] shr 8)];
  W2 := ForwardTable[byte(T0[0] shr 16)];
  W3 := ForwardTable[byte(T0[1] shr 24)];
  T1[2] := (W0 xor ((W1 shl 8) or (W1 shr 24)) xor ((W2 shl 16) or (W2 shr 16)) xor
    ((W3 shl 24) or (W3 shr 8))) xor Key[30];
  W0 := ForwardTable[byte(T0[3])];
  W1 := ForwardTable[byte(T0[0] shr 8)];
  W2 := ForwardTable[byte(T0[1] shr 16)];
  W3 := ForwardTable[byte(T0[2] shr 24)];
  T1[3] := (W0 xor ((W1 shl 8) or (W1 shr 24)) xor ((W2 shl 16) or (W2 shr 16)) xor
    ((W3 shl 24) or (W3 shr 8))) xor Key[31];
  // round 8
  W0 := ForwardTable[byte(T1[0])];
  W1 := ForwardTable[byte(T1[1] shr 8)];
  W2 := ForwardTable[byte(T1[2] shr 16)];
  W3 := ForwardTable[byte(T1[3] shr 24)];
  T0[0] := (W0 xor ((W1 shl 8) or (W1 shr 24)) xor ((W2 shl 16) or (W2 shr 16)) xor
    ((W3 shl 24) or (W3 shr 8))) xor Key[32];
  W0 := ForwardTable[byte(T1[1])];
  W1 := ForwardTable[byte(T1[2] shr 8)];
  W2 := ForwardTable[byte(T1[3] shr 16)];
  W3 := ForwardTable[byte(T1[0] shr 24)];
  T0[1] := (W0 xor ((W1 shl 8) or (W1 shr 24)) xor ((W2 shl 16) or (W2 shr 16)) xor
    ((W3 shl 24) or (W3 shr 8))) xor Key[33];
  W0 := ForwardTable[byte(T1[2])];
  W1 := ForwardTable[byte(T1[3] shr 8)];
  W2 := ForwardTable[byte(T1[0] shr 16)];
  W3 := ForwardTable[byte(T1[1] shr 24)];
  T0[2] := (W0 xor ((W1 shl 8) or (W1 shr 24)) xor ((W2 shl 16) or (W2 shr 16)) xor
    ((W3 shl 24) or (W3 shr 8))) xor Key[34];
  W0 := ForwardTable[byte(T1[3])];
  W1 := ForwardTable[byte(T1[0] shr 8)];
  W2 := ForwardTable[byte(T1[1] shr 16)];
  W3 := ForwardTable[byte(T1[2] shr 24)];
  T0[3] := (W0 xor ((W1 shl 8) or (W1 shr 24)) xor ((W2 shl 16) or (W2 shr 16)) xor
    ((W3 shl 24) or (W3 shr 8))) xor Key[35];
  // round 9
  W0 := ForwardTable[byte(T0[0])];
  W1 := ForwardTable[byte(T0[1] shr 8)];
  W2 := ForwardTable[byte(T0[2] shr 16)];
  W3 := ForwardTable[byte(T0[3] shr 24)];
  T1[0] := (W0 xor ((W1 shl 8) or (W1 shr 24)) xor ((W2 shl 16) or (W2 shr 16)) xor
    ((W3 shl 24) or (W3 shr 8))) xor Key[36];
  W0 := ForwardTable[byte(T0[1])];
  W1 := ForwardTable[byte(T0[2] shr 8)];
  W2 := ForwardTable[byte(T0[3] shr 16)];
  W3 := ForwardTable[byte(T0[0] shr 24)];
  T1[1] := (W0 xor ((W1 shl 8) or (W1 shr 24)) xor ((W2 shl 16) or (W2 shr 16)) xor
    ((W3 shl 24) or (W3 shr 8))) xor Key[37];
  W0 := ForwardTable[byte(T0[2])];
  W1 := ForwardTable[byte(T0[3] shr 8)];
  W2 := ForwardTable[byte(T0[0] shr 16)];
  W3 := ForwardTable[byte(T0[1] shr 24)];
  T1[2] := (W0 xor ((W1 shl 8) or (W1 shr 24)) xor ((W2 shl 16) or (W2 shr 16)) xor
    ((W3 shl 24) or (W3 shr 8))) xor Key[38];
  W0 := ForwardTable[byte(T0[3])];
  W1 := ForwardTable[byte(T0[0] shr 8)];
  W2 := ForwardTable[byte(T0[1] shr 16)];
  W3 := ForwardTable[byte(T0[2] shr 24)];
  T1[3] := (W0 xor ((W1 shl 8) or (W1 shr 24)) xor ((W2 shl 16) or (W2 shr 16)) xor
    ((W3 shl 24) or (W3 shr 8))) xor Key[39];
  ...
  T0[0] := (W0 xor ((W1 shl 8) or (W1 shr 24)) xor ((W2 shl 16) or (W2 shr 16)) xor
    ((W3 shl 24) or (W3 shr 8))) xor Key[40];
  ...
  T0[1] := (W0 xor ((W1 shl 8) or (W1 shr 24)) xor ((W2 shl 16) or (W2 shr 16)) xor
    ((W3 shl 24) or (W3 shr 8))) xor Key[41];
  ...
  T0[2] := (W0 xor ((W1 shl 8) or (W1 shr 24)) xor ((W2 shl 16) or (W2 shr 16)) xor
    ((W3 shl 24) or (W3 shr 8))) xor Key[42];
  ...
  T0[3] := (W0 xor ((W1 shl 8) or (W1 shr 24)) xor ((W2 shl 16) or (W2 shr 16)) xor
    ((W3 shl 24) or (W3 shr 8))) xor Key[43];
  // finalizing
...
begin
  ...
  W3 := ForwardTable[byte(T0[3] shr 24)];
  T1[0] := (W0 xor ((W1 shl 8) or (W1 shr 24)) xor ((W2 shl 16) or (W2 shr 16)) xor
    ((W3 shl 24) or (W3 shr 8))) xor Key[44];
  W0 := ForwardTable[byte(T0[1])];
  W1 := ForwardTable[byte(T0[2] shr 8)];
  W2 := ForwardTable[byte(T0[3] shr 16)];
  W3 := ForwardTable[byte(T0[0] shr 24)];
  T1[1] := (W0 xor ((W1 shl 8) or (W1 shr 24)) xor ((W2 shl 16) or (W2 shr 16)) xor
    ((W3 shl 24) or (W3 shr 8))) xor Key[45];
  W0 := ForwardTable[byte(T0[2])];
  W1 := ForwardTable[byte(T0[3] shr 8)];
  W2 := ForwardTable[byte(T0[0] shr 16)];
  W3 := ForwardTable[byte(T0[1] shr 24)];
  T1[2] := (W0 xor ((W1 shl 8) or (W1 shr 24)) xor ((W2 shl 16) or (W2 shr 16)) xor
    ((W3 shl 24) or (W3 shr 8))) xor Key[46];
  W0 := ForwardTable[byte(T0[3])];
  W1 := ForwardTable[byte(T0[0] shr 8)];
  W2 := ForwardTable[byte(T0[1] shr 16)];
  W3 := ForwardTable[byte(T0[2] shr 24)];
  T1[3] := (W0 xor ((W1 shl 8) or (W1 shr 24)) xor ((W2 shl 16) or (W2 shr 16)) xor
    ((W3 shl 24) or (W3 shr 8))) xor Key[47];
  // last round of transformations
  W0 := LastForwardTable[byte(T1[0])];
  W1 := LastForwardTable[byte(T1[1] shr 8)];
  W2 := LastForwardTable[byte(T1[2] shr 16)];
  W3 := LastForwardTable[byte(T1[3] shr 24)];
  T0[0] := (W0 xor ((W1 shl 8) or (W1 shr 24)) xor ((W2 shl 16) or (W2 shr 16)) xor
    ((W3 shl 24) or (W3 shr 8))) xor Key[48];
  W0 := LastForwardTable[byte(T1[1])];
  W1 := LastForwardTable[byte(T1[2] shr 8)];
  W2 := LastForwardTable[byte(T1[3] shr 16)];
  W3 := LastForwardTable[byte(T1[0] shr 24)];
  T0[1] := (W0 xor ((W1 shl 8) or (W1 shr 24)) xor ((W2 shl 16) or (W2 shr 16)) xor
    ((W3 shl 24) or (W3 shr 8))) xor Key[49];
  W0 := LastForwardTable[byte(T1[2])];
  W1 := LastForwardTable[byte(T1[3] shr 8)];
  W2 := LastForwardTable[byte(T1[0] shr 16)];
  W3 := LastForwardTable[byte(T1[1] shr 24)];
  T0[2] := (W0 xor ((W1 shl 8) or (W1 shr 24)) xor ((W2 shl 16) or (W2 shr 16)) xor
    ((W3 shl 24) or (W3 shr 8))) xor Key[50];
  W0 := LastForwardTable[byte(T1[3])];
  W1 := LastForwardTable[byte(T1[0] shr 8)];
  W2 := LastForwardTable[byte(T1[1] shr 16)];
  W3 := LastForwardTable[byte(T1[2] shr 24)];
  T0[3] := (W0 xor ((W1 shl 8) or (W1 shr 24)) xor ((W2 shl 16) or (W2 shr 16)) xor
    ((W3 shl 24) or (W3 shr 8))) xor Key[51];
  // finalizing
  PLongWord(@OutBuf[0])^ := T0[0];
  PLongWord(@OutBuf[4])^ := T0[1];
  PLongWord(@OutBuf[8])^ := T0[2];
...
begin
  ...
  W3 := ForwardTable[byte(T0[3] shr 24)];
  T1[0] := (W0 xor ((W1 shl 8) or (W1 shr 24)) xor ((W2 shl 16) or (W2 shr 16)) xor
    ((W3 shl 24) or (W3 shr 8))) xor Key[52];
  W0 := ForwardTable[byte(T0[1])];
  W1 := ForwardTable[byte(T0[2] shr 8)];
  W2 := ForwardTable[byte(T0[3] shr 16)];
  W3 := ForwardTable[byte(T0[0] shr 24)];
  T1[1] := (W0 xor ((W1 shl 8) or (W1 shr 24)) xor ((W2 shl 16) or (W2 shr 16)) xor
    ((W3 shl 24) or (W3 shr 8))) xor Key[53];
  W0 := ForwardTable[byte(T0[2])];
  W1 := ForwardTable[byte(T0[3] shr 8)];
  W2 := ForwardTable[byte(T0[0] shr 16)];
  W3 := ForwardTable[byte(T0[1] shr 24)];
  T1[2] := (W0 xor ((W1 shl 8) or (W1 shr 24)) xor ((W2 shl 16) or (W2 shr 16)) xor
    ((W3 shl 24) or (W3 shr 8))) xor Key[54];
  W0 := ForwardTable[byte(T0[3])];
  W1 := ForwardTable[byte(T0[0] shr 8)];
  W2 := ForwardTable[byte(T0[1] shr 16)];
  W3 := ForwardTable[byte(T0[2] shr 24)];
  T1[3] := (W0 xor ((W1 shl 8) or (W1 shr 24)) xor ((W2 shl 16) or (W2 shr 16)) xor
    ((W3 shl 24) or (W3 shr 8))) xor Key[55];
  // last round of transformations
  W0 := LastForwardTable[byte(T1[0])];
  W1 := LastForwardTable[byte(T1[1] shr 8)];
  W2 := LastForwardTable[byte(T1[2] shr 16)];
  W3 := LastForwardTable[byte(T1[3] shr 24)];
  T0[0] := (W0 xor ((W1 shl 8) or (W1 shr 24)) xor ((W2 shl 16) or (W2 shr 16)) xor
    ((W3 shl 24) or (W3 shr 8))) xor Key[56];
  W0 := LastForwardTable[byte(T1[1])];
  W1 := LastForwardTable[byte(T1[2] shr 8)];
  W2 := LastForwardTable[byte(T1[3] shr 16)];
  W3 := LastForwardTable[byte(T1[0] shr 24)];
  T0[1] := (W0 xor ((W1 shl 8) or (W1 shr 24)) xor ((W2 shl 16) or (W2 shr 16)) xor
    ((W3 shl 24) or (W3 shr 8))) xor Key[57];
  W0 := LastForwardTable[byte(T1[2])];
  W1 := LastForwardTable[byte(T1[3] shr 8)];
  W2 := LastForwardTable[byte(T1[0] shr 16)];
  W3 := LastForwardTable[byte(T1[1] shr 24)];
  T0[2] := (W0 xor ((W1 shl 8) or (W1 shr 24)) xor ((W2 shl 16) or (W2 shr 16)) xor
    ((W3 shl 24) or (W3 shr 8))) xor Key[58];
  W0 := LastForwardTable[byte(T1[3])];
  W1 := LastForwardTable[byte(T1[0] shr 8)];
  W2 := LastForwardTable[byte(T1[1] shr 16)];
  W3 := LastForwardTable[byte(T1[2] shr 24)];
  T0[3] := (W0 xor ((W1 shl 8) or (W1 shr 24)) xor ((W2 shl 16) or (W2 shr 16)) xor
    ((W3 shl 24) or (W3 shr 8))) xor Key[59];
  // finalizing
  PLongWord(@OutBuf[0])^ := T0[0];
  PLongWord(@OutBuf[4])^ := T0[1];
  PLongWord(@OutBuf[8])^ := T0[2];
...
begin
  ...
  T0[0] := (W0 xor ((W1 shl 8) or (W1 shr 24)) xor ((W2 shl 16) or (W2 shr 16)) xor
    ((W3 shl 24) or (W3 shr 8))) xor Key[0];
  ...
  T0[1] := (W0 xor ((W1 shl 8) or (W1 shr 24)) xor ((W2 shl 16) or (W2 shr 16)) xor
    ((W3 shl 24) or (W3 shr 8))) xor Key[1];
  ...
  T0[2] := (W0 xor ((W1 shl 8) or (W1 shr 24)) xor ((W2 shl 16) or (W2 shr 16)) xor
    ((W3 shl 24) or (W3 shr 8))) xor Key[2];
  ...
  T0[3] := (W0 xor ((W1 shl 8) or (W1 shr 24)) xor ((W2 shl 16) or (W2 shr 16)) xor
    ((W3 shl 24) or (W3 shr 8))) xor Key[3];
  // finalizing
  PLongWord(@OutBuf[0])^ := T0[0];
  PLongWord(@OutBuf[4])^ := T0[1];
  PLongWord(@OutBuf[8])^ := T0[2];
...
procedure EncryptAESStreamCBC(Source: TStream; Count: cardinal;
  const ExpandedKey: TAESExpandedKey128; const InitVector: TAESBuffer; Dest: TStream);
var
...
procedure EncryptAESStreamCBC(Source: TStream; Count: cardinal;
  const ExpandedKey: TAESExpandedKey192; const InitVector: TAESBuffer; Dest: TStream);
var
  TempIn, TempOut, Vector: TAESBuffer;
  Done: cardinal;
begin
...
procedure EncryptAESStreamCBC(Source: TStream; Count: cardinal;
  const ExpandedKey: TAESExpandedKey256; const InitVector: TAESBuffer; Dest: TStream);
var
  TempIn, TempOut, Vector: TAESBuffer;
  Done: cardinal;
begin
...
procedure TAkeebaAES.AESSetPassword(password: string);
var
  key: TCTRMessage;
  ctr: TAESCTR;
  i: integer;
